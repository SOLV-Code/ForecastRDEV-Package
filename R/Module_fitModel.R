...
#' @param data.obj full list object generated by prepData() (move everything over to this, then remove data and data.sibreg arguments
...
out.list <-  model.functions.src[[model]]$fitting(data. = data.obj,settings. = settings,tracing. = tracing)
...
if(any(is.na(ages))){
	...
	out.list[["Total"]] <- model.functions.src[[model]]$estimator(model.data = data.in,avg.yrs=settings$avg.yrs)
...
if(!any(is.na(ages))){  # if have age classes, loop through them
	for(age.do in ages){
		...
		out.list[[paste(age.prefix,age.do,sep="")]] <- model.functions.src[[model]]$estimator(model.data = data.in,avg.yrs=settings$avg.yrs)
...
if(model %in%  c("ReturnRate")){
  ...
  # too many variations, so just forcing all or nothing settings
	...
	if(any(is.na(ages))){
		...
		out.list[["Total"]] <- model.functions.src[[model]]$estimator(data.use = data.in,avg=avg.use, pred.label = pred.label.use, last.n  = last.n.use)
	} # end if no age classes
	...
	if(!any(is.na(ages))){  # if have age classes, loop through them
		for(age.do in ages){
   		...
   		out.list[[paste(age.prefix,age.do,sep="")]] <- model.functions.src[[model]]$estimator(data.use = data.in,avg=avg.use, pred.label = pred.label.use, last.n  = last.n.use)
...
out.list[[paste(age.prefix,age.do,sep="")]] <- model.functions.src[["Naive"]]$estimator(model.data = data.in,avg.yrs=5)
}
...
out.list[[paste(age.prefix,age.do,sep="")]] <- c(model.functions.src[[model]]$estimator(data.in,settings=settings) ,
...
if(any(is.na(ages))){
	data.in <- data[["Total"]][,2] # FIX to dynamic col label
	names(data.in) <- data[["Total"]][,1] # FIX to dynamic col label
	out.list[["Total"]] <- c(model.functions.src[[model]]$estimator(model.data = data.in,settings = settings),
...
if(!any(is.na(ages))){  # if have age classes, loop through them
	for(age.do in ages){
		...
		out.list[[paste(age.prefix,age.do,sep="")]] <- c(model.functions.src[[model]]$estimator(model.data = data.in,settings = settings),
...
#	if(any(is.na(ages))){
		data.in <- data[["Total"]]
		out.list[["Total"]] <- c(model.functions.src[[model]]$estimator(data.in,settings = settings),
														 list(run.yrs = data[["Total"]][,1]))
